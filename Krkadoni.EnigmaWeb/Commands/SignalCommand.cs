using System.Runtime.InteropServices;
using System.Threading;
using System.Threading.Tasks;
using Krkadoni.Enigma.Enums;
using Krkadoni.Enigma.Parsers;
using Krkadoni.Enigma.Properties;
using Krkadoni.Enigma.Responses;

namespace Krkadoni.Enigma.Commands
{
    [ComVisible(false)]
    public class SignalCommand : EnigmaCommand<ISignalCommand, ISignalResponse>, ISignalCommand
    {
        private readonly IResponseParser<ISignalCommand, ISignalResponse> _parser;

        public SignalCommand([NotNull] IFactory factory)
            : base(factory)
        {
            _parser = Factory.SignalParser();
        }

        public async Task<ISignalResponse> ExecuteAsync(IProfile profile, CancellationToken token)
        {
            string url = profile.Enigma == EnigmaType.Enigma1 ? @"satFinder" : @"web/signal";
            return await base.ExecuteAsync(profile, url, _parser, token);
        }
    }
}